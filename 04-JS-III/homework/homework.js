...
function promedioResultadosTest(resultadosTest) {
    ...
    // Tu código:
    let promedio = resultadosTest.reduce(function(a, b) {
        return a + b;
    }, 0);
...
function cuentoElementos(arreglo) {
    ...
    arreglo.forEach((element) => {
        if (element > 18) {
            cantidadElementos.push(element);
        }
    });
    return cantidadElementos.length;
...
function mesesDelAño(array) {
    ...
    array.forEach(element => {
        if (element === 'Enero') meses.push(element);

        if (element === 'Marzo') meses.push(element);

        if (element === 'Noviembre') meses.push(element);
    });
